...
import { MagicWandIcon, Cross2Icon, CubeIcon, ReloadIcon, CheckCircledIcon, ExclamationTriangleIcon, ImageIcon } from "@radix-ui/react-icons";
    ...
    <div className="absolute right-4 top-4 z-20 w-[420px] rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 shadow-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-base">Generate Architecture</h3>
            <p className="text-white/50 text-xs">Text → 2D → 3D Pipeline</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          <label className="text-white/70 text-sm font-medium">Describe the Building</label>
          <textarea
            value={prompt}
            ...
            className="w-full h-24 px-4 py-3 rounded-lg bg-black/40 border border-white/10 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 resize-none text-sm"
        ...
        {/* Quick Prompts */}
        <div className="space-y-2">
          <label className="text-white/70 text-sm font-medium">Quick Styles</label>
        ...
        {/* Style Selection */}
        <div className="space-y-2">
          <label className="text-white/70 text-sm font-medium">Architecture Style</label>
        ...
        {/* Views Selection */}
        <div className="space-y-2">
          <label className="text-white/70 text-sm font-medium">Number of Views</label>
          ...
          <p className="text-white/40 text-xs">More views = better 3D quality, but slower generation</p>
        </div>
            ...
            <div className="space-y-2">
              {stages.map((stage, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  {stage.status === "active" && (
                    <ReloadIcon className="w-4 h-4 text-cyan-400 animate-spin" />
                  )}
          ...
          >
            {isGenerating ? (
              ...
              <>
                ...
                <span>Generate 3D Architecture</span>
              </>
            )}
          </button>
        )}
